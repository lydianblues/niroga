	...
	vc.AddElementUIPanelBackendEditor = vc.PanelView
		...
		.extend( {
			...
			createElement: function ( e ) {
				...
				var model, column, row, showSettings, page_section, shortcode,
					tag = $( e.currentTarget ).data( 'tag' );
					
				if ( false === this.model ) {
					vc.storage.lock();
					if('mk_page_section' == tag) {
						page_section = vc.shortcodes.create( { shortcode: 'mk_page_section' } );
						column = vc.shortcodes.create( {
							shortcode: 'vc_column',
							params: { width: '1/1' },
							parent_id: page_section.id,
							root_id: page_section.id
						} );
					} else {
					...
					if ( 'vc_row' !== tag && 'mk_page_section' !== tag ) {
